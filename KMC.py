...
if __name__ == "__main__":
	...
	ap.add_argument("-c", "--clusters", type = int, default = 4,
	...
	result = True #convert(arg) # True for test
	if (result):
	...
	print("Silhouette Coefficient: %0.3f" % metrics.silhouette_score(data, km.labels_, sample_size=1000))
	print("")
	...
	for (l, c) in itertools.izip_longest(labels, clusters): 
		if (l == 'white'):				# white
	...
	rects1 = plt.bar(index, means_white, bar_width, alpha=opacity, color='g', label='white')
	rects2 = plt.bar(index + bar_width, means_mal, bar_width, alpha=opacity, color='r', label='mal')
